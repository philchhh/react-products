...
export default async (req, res) => {
  ...
  try {
    ...
    // Calculate cart totals again from cart products (from DB)
    const { cartTotal, stripeTotal } = calculateCartTotal(cart.products);

    // Get email from payment data, see if it's linked with existing Stripe customer
    ...
    // Clear existing products in cart
    await Cart.findOneAndUpdate({ _id: cart._id }, { $set: { products: [] } });

    // Send back success(200) response
    res.status(200).send("Checkout sccessful!");
  } catch (error) {
    console.error(error);
